...
namespace App\Http\Requests\JefeContabilidad;
...
class UpdateJefeContabilidadRequest extends FormRequest
{
    /**
    ...
    {
        // 1. Obtenemos el ID del jefe de contabilidad desde la ruta 
        $jefeContabilidadId = $this->route('id'); 
        
        // 2. Buscamos el jefe de contabilidad y los IDs de sus tablas relacionadas
        try {
            $jefeContabilidad = JefeContabilidad::with('datos.contacto')->findOrFail($jefeContabilidadId);
            $datosId = $jefeContabilidad->datos->id;
            // Verificamos si el contacto existe antes de tomar su ID
            $contactoId = $jefeContabilidad->datos->contacto ? $jefeContabilidad->datos->contacto->id : null;
        ...
        return [
            ...
            'usuario.username' => [
                ...
                // Ignora este 'jefeContabilidadId' (que es un ID de 'usuarios')
                Rule::unique('usuarios', 'username')->ignore($jefeContabilidadId),
            ],
